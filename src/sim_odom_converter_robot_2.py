...
def posecallback(data):
    ...
    for k in range(0, len(data.name)):
        if (data.name[k] == "robot_2::base_footprint"):
    ...
    if (len(data.pose) > 1):
        ...
        odom.pose.pose.position.x -= 4.99995139
        ...
        odom.child_frame_id = "robot_2/base_footprint"
        #odom.child_frame_id = "dummy_link"

        pub = rospy.Publisher('/robot_2/odom', Odometry, queue_size=10)
        ...
        t.child_frame_id = "robot_2/base_footprint"
...
def main():
    rospy.init_node('sim_odom_converter_2', anonymous=True)
